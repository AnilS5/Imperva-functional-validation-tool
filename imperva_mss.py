...
def imperva(email_id, imprv_url, dom_test):
    ...
    #wait.until(md.EC.presence_of_element_located((md.b.ID, 'temp-click')))
    #findElement(md.b.ID, 'email').send_keys(email_id)
    #findElement(md.b.ID, 'temp-click').click()
    ...
    md.sleep(5)
    findElement(md.b.XPATH, "//*[contains(text(), 'Switch to Airlink MSS')]").click()
    md.sleep(5)
    wait.until(md.EC.presence_of_element_located((md.b.ID, 'currentAccountWrapper')))
    findElement(md.b.ID, 'currentAccountWrapper').click()
    wait.until(md.EC.presence_of_element_located((md.b.CSS_SELECTOR, 'div[title="Amadeus Airlink"]')))
    findElement(md.b.CSS_SELECTOR, 'div[title="Amadeus Airlink"]').click()
    driver.fullscreen_window()

    driver.switch_to.default_content()
    for dom in dom_test:
        #image_name = str(dom).replace(".", "-")
        image_name = str(dom)
        ...
        # try:
        #     md.sleep(5)
        #     cert_data = 'No certs found.'
        #     findElement(md.b.CSS_SELECTOR, "a[title='Details']").click()
        #     wait.until(md.EC.visibility_of_element_located((md.b.ID, 'exisitingCustomCertificateDetailsPane')))
        #     driver.save_screenshot(md.path.join('Images/', 'certdata' + image_name + '.png'))
        #     cert_data = driver.find_element(md.b.ID, 'exisitingCustomCertificateDetailsPane').text
        #     findElement(md.b.ID, 'customCertificateDisplayClosebutton').click()
        # except md.NoSuchElementException:
        #     pass
        ...
        driver.save_screenshot(md.path.join('Images/', 'certdata' + image_name + '_1.png'))
